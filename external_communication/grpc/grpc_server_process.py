...
def grpc_server_process(action_queue: Queue, player_turn):
    try:
        server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
        relay_node_pb2_grpc.add_RelayNodeServicer_to_server(RelayNodeServicer(action_queue, player_turn), server)
